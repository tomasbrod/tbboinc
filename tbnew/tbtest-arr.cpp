...
struct DLX : public cfg {
	...
	void init_heads () {
		...
		node[0].left = nheads; // first points to last
		node[nheads].right = 0; // last points to first
	}
	void link_node_init(Index i, Index t) {
		...
		n.top = n.down = t;
		n.up = node[t].up;
		node[t].up = node[node[t].up].down = i;
		node[t].count ++;
	}
	Index choose_column() {
		Index c = node[nheads].right;
		int s = node[c].count;
		for (Index j = c; j!=nheads; j=node[j].right) {
			if( node[j].count < s ) {
				s = node[j].count;
				if(!s) return j;
				c = j;
	...
	void cover(Index n) {
		...
		do {
			...
			for(Index r = node[c].down; r!=c; r = node[r].down) {
				for(Index k = node[r].right; k!=r; k = node[k].right) {
					//cout<<" unlink node "<<k<<" u"<<node[k].up<<" d"<<node[k].down<<endl;
					//if((node[node[k].up].down!=k)||(node[node[k].down].up!=k)) link_error();
					node[node[k].up].down = node[k].down;
					node[node[k].down].up = node[k].up;
					node[node[k].top].count -- ;
				}
			}
			j = node[j].right;
		} while(j!=n);
	}
	void uncover(Index n) {
		Index j = n;
		do {
			j = node[j].left;
			Index c = node[j].top;
			for(Index r = node[c].up; r!=c; r = node[r].up) {
				for(Index k = node[r].left; k!=r; k = node[k].left) {
					//cout<<" link node "<<k<<" u"<<node[k].up<<" d"<<node[k].down<<endl;
					node[node[k].down].up = k;
					node[node[k].up].down = k;
					node[node[k].top].count ++ ;
				}
			}
	...
	bool  __attribute__ ((noinline)) search () {
		...
		advance:
			...
			if( c == nheads ) return true; // no more columns -> solved
			r = node[c].down;	// select first available row
		...
		backtrack:
			...
			c= node[r].top;
			uncover(r);
			r = node[r].down; // select next available row
			goto descend;
	}
};
...
struct trans_dlx_cfg {
	...
	std::array<Node, max_trans*10 + 101 > node;
	std::array<Index, 10 > path; // solution path
...
void init_trans(const kvadrat& lk) {
	...
	for(int i = 0; i < raz; i++){
		...
		if(temp[3]<10){
			node[count+3].right = count; // loop back forwards
			node[count].left += 4; // loop front backwards
			count += 4;
		}
		else{
			node[count+2].right = count;
			node[count].left += 3;
