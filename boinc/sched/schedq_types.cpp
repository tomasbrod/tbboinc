...
const char* SCHEDULER_REQUEST::parse(XML_PARSER& xp) {
    ...
    while (!xp.get_tag()) {
        ...
        if (xp.match_tag("time_stats_log")) {
			std::string buf;
			//FIXME: how are we supposed to handle them??
			int retval = copy_element_contents(*xp.f, "</time_stats_log>", buf);
            if (retval) {
                log_messages.printf(MSG_NORMAL,
                    "SCHEDULER_REQUEST::parse(): Couldn't parse contents of <time_stats_log>. Ignoring it.");
            } else {
                have_time_stats_log = false; //true
            }
            continue;
        }
...
int SCHEDULER_REPLY::write(FILE* fout, SCHEDULER_REQUEST& sreq) {
    ...
    if (config.request_time_stats_log) {
        if (/*FIXME: !have_time_stats_log()*/ 0) {
    ...
    //
	//FIXME: get this info from the object, not ssp/db here.
    #if 0
...
int APP_VERSION::write(FILE* fout) {
    ...
    *p = 0;
	...
	//FIXME: get the platform name somehow
