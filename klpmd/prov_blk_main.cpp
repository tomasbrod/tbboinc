...
void vyvod(){
	...
	cout << "Найдено КФ марьяжных ДЛК: " << baza_mar.size() << endl;
}

int main(int argc, char* argv[]){
	setlocale(LC_CTYPE, "rus");
	cout << "Поиск всех КФ марьяжных ДЛК для ЛК\n\n";
	input = argc == 1 ? "input.txt" : argv[1];
	if(init()){
		clock_t t0 = clock(), tb, te;
		tb = t0;
		int count = 0, temp_count = 0, temp;
		for(auto q = baza_lk.begin(); q != baza_lk.end(); q++){
			count++;
			rabota(*q);
			if((temp = (te = clock()) - tb) > timeout){
				cout << "Проверено ЛК: " << count << " скорость "
					<< int(double(count_dlk - temp_count) / temp * CLOCKS_PER_SEC + 0.5) << " ДЛК в сек\n";
				temp_count = count_dlk;
				tb = te;
			}
		}
		cout << "\nПроверено ДЛК           : " << count_dlk << endl;
		if(baza_mar.empty()) cout << "Марьяжных ДЛК нет\n";
		else vyvod();
